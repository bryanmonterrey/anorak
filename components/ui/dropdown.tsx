...
>(({ className, sideOffset = 8, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cnExt(
        'z-50 w-[300px] overflow-hidden bg-zinc-950 border border-zinc-800/75 mr-2 rounded-3xl p-2 shadow-md',
        'flex flex-col gap-1',
        // origin
...
>(({ className, inset, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Item
    ...
    className={cnExt(
      // base
      'group/item shadow-md relative cursor-pointer w-full select-none rounded-lg p-1 text-paragraph-sm text-text-strong-950 outline-none',
      'flex items-center',
...
>(({ className, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Group
    ref={forwardedRef}
    className={cnExt('flex flex-col', className)}
    {...rest}
  />
));
